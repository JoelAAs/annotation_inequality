...
print(f"Processing GO {aspect} Depth {depth}, Cutoff {cutoff} Plotting...")
...
if df.empty or (df['Coefficient'] == 0).all():
    ...
    for out in [output_top, output_distribution]:
        ...
        plt.text(0.5, 0.5, f"No significant GO ids found\n(Depth {depth}, Cutoff {cutoff})", 
...
try:
    ...
    print(f"Assigning GO terms for depth {depth}, cutoff {cutoff} to their respective GO ids...")
    goid_to_name = {term.id: term.name for term in ontology.terms()} 
    df['GO_term'] = df['GO_id'].map(goid_to_name).fillna(df['GO_id'])
    print(f"Terms for {aspect} depth {depth}, cutoff {cutoff} assigned!\n")
except Exception as e:
    print(f"Error parsing ontology file: {e}. Falling back to raw IDs.")
    df['GO_term'] = df['GO_id']

# Handle specific labels
df['GO_term'] = df['GO_term'].replace('No_go_id', 'No_go_id_assigned')

# Plot Top Coefficients
print(f"Generating {aspect} depth {depth}, cutoff {cutoff} Top Coefficients plot...")
...
plt.barh(df_plot['GO_term'], df_plot['Coefficient'], color=colors, alpha=0.8)
...
plt.title(f'Top Influential GO {aspect} ids: Depth {depth}, Cutoff {cutoff}\n(Green = Positive Association, Red = Negative)', 
          fontsize=16, pad=20)
...
print(f"{aspect} Depth {depth}, cutoff {cutoff} Top Coefficients plot ready!\n")

# Plot Coefficient Distribution
print(f"Generating {aspect} depth {depth}, cutoff {cutoff} Distribution plot...")
...
plt.title(f'Distribution of GO {aspect} Coefficients: Depth {depth}, Cutoff {cutoff}', fontsize=15)
...
print(f"GO {aspect} Depth {depth}, cutoff {cutoff} Distribution plot ready!\n")

print(f"GO {aspect} Depth {depth}, Cutoff {cutoff} plots saved successfully.")
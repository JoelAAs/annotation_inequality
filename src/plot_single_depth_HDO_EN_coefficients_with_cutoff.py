...
print(f"Processing depth {depth} HDO coefficients with cutoff {cutoff} plotting...\n")
...
if df.empty:
    print(f"WARNING: No valid coefficients found for depth {depth} cutoff {cutoff}. Creating empty files.")
    # Create empty plots or exit gracefully so snakemake doesn't hang
    plt.figure().savefig(output_top)
    plt.figure().savefig(output_distribution)
    sys.exit(0)
...
print(f"Plotting Top coefficients for depth {depth} cutoff {cutoff}...")
...
plt.title(f'Top depth {depth} HDO full Elastic Net Coefficients with cutoff {cutoff} (Green = Positive, Red = Negative)')
...
print(f"Depth {depth} Cutoff {cutoff} Top coefficients plotted!\n")

print(f"Plotting coefficients distribution for depth {depth} cutoff {cutoff}...\n")
...
plt.title(f'Histogram of depth {depth} complete HDO full Elastic Net Coefficients')
plt.legend([f'Total coefficients = {total_coefs}'], loc='upper right')
plt.savefig(output_distribution, dpi = 300)
plt.close()

print(f"Depth {depth} Cutoff {cutoff} Coefficient distribution plotted!\n")

print(f"Depth {depth} HDO coefficients with cutoff {cutoff} plotting done!")
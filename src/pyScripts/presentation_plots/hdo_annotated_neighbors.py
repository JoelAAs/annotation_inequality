...
print(f"Processing top HDO coefficients depth {depth} cutoff {cutoff} annotated neighbors correlation with baseline plotting...")

print(f"Loading top HDO coefficients depth {depth} cutoff {cutoff} annotated neighbors correlation with baseline data...")
...
print(f"Setting up top HDO coefficients depth {depth} cutoff {cutoff} annotated neighbors correlation with baseline data...")
...
print(f"Plotting top HDO coefficients depth {depth} cutoff {cutoff} annotated neighbors correlation with baseline...")
...
sns.violinplot(
    ...
    ax = ax,
    linewidth=2,
    order=stats['HDO_term'] # Ensures the x-axis order exactly matches the iteration order for p-values
)
...
for i, row in stats.iterrows():
    ...
    # Find the maximum value for this specific HDO term to place text right above the violin
    term_max = dist_df[dist_df['HDO_term'] == row['HDO_term']]['Value'].max()
...
ax.set_title(f"Distribution of HDO Annotated Neighbors (Log Scale)", loc = 'left', fontweight='bold', fontsize = 26, pad = 25)
ax.set_ylabel("Annotated neighbors (log scale)", fontsize = 22, labelpad = 20)
ax.set_xlabel("HDO Annotation", fontsize = 22, labelpad = 20)
...
print(f"Saving Top HDO coefficients depth {depth} cutoff {cutoff} annotated neighbors correlation with baseline plots...")

plt.savefig(outputplots, dpi = 300, bbox_inches='tight')

print(f"Top HDO coefficients depth {depth} cutoff {cutoff} annotated neighbors correlation with baseline plotting done!")
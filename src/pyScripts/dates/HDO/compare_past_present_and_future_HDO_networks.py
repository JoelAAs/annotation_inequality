...
print(f"Computing across-time networks statistics for HDO depth {depth} cutoff {cutoff}...")
...
print(f"Processing HDO depth {depth} cutoff {cutoff} temporal neighbor statistics...")
...
for index, row in top_annot.iterrows():
    ...
    for target_node in genes:
        target_date = get_annotation_date(target_node, do_id)
        ...
        for neighbor in G.neighbors(target_node):
            ...
            neighbor_date = get_annotation_date(neighbor, do_id)
...
print(f"Saving across-time networks statistics for HDO depth {depth} cutoff {cutoff}...")
stats_df.to_csv(outputstatistics, sep='\t', index=False)
print(f"Across-time networks statistics for HDO depth {depth} cutoff {cutoff} ready!")
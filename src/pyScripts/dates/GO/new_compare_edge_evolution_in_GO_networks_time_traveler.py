...
print(f"Computing Node-Centric Mean Fractions (RETROSPECTIVE Logic) for GO {aspect} depth {depth} cutoff {cutoff}...")
...
for index, row in top_annot.iterrows():
    ...
    for target_node in genes:
        ...
        for neighbor in G.neighbors(target_node):
            ...
            # RETROSPECTIVE LOGIC: 
            # If the neighbor ever gets this annotation, it counts as annotated 
            # the exact moment the edge was discovered
            neighbor_annot_int = get_annotation_date(neighbor, go_id)
            if neighbor_annot_int is not None:
                # Event B: Neighbor is annotated (Numerator)
                gene_events[m_edge]['annot_added'] += 1
...
print("Node-Centric Mean Time-Zero statistics ready for GO (Retrospective)!")
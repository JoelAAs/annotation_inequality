...
print(f"Generating multi-page PDF for baseline GO {aspect} depth {depth} cutoff {cutoff} neighbors bait count sums...\n")

print(f"Loading data for baseline GO {aspect} depth {depth} cutoff {cutoff} neighbors bait count sums...")

with open(neighbors_sums_df, 'rb') as f:
    df = pickle.load(f)

print(f"baseline GO {aspect} depth {depth} cutoff {cutoff} neighbors bait count sums data loaded!\n")

print(f"Plotting baseline GO {aspect} depth {depth} cutoff {cutoff} neighbors bait count sums...")
...
with PdfPages(outputpdf) as pdf:
    ...
    for index, row in df.iterrows():
        ...
        plt.title(f'Frequency of GO {aspect} Baseline Neighbor Bait Count Sums\nAnnotation: {goid} | Genes with annotation: {len(sums)}', fontsize = 14)
...
print(f"Baseline GO {aspect} depth {depth} cutoff {cutoff} neighbors bait count sums plotted!\n")

print(f"Multi-page PDF for baseline GO {aspect} depth {depth} cutoff {cutoff} neighbors bait count sums ready!\n")
...
go_ids = agg_df['GO_id'].unique()
colors = sns.color_palette("viridis", len(go_ids))
color_map = {go_id: colors[i] for i, go_id in enumerate(go_ids)}

for i, go_id in enumerate(go_ids):
    row = agg_df[agg_df['GO_id'] == go_id].iloc[0]
    ...
    if valid_mask.sum() == 3:
        plt.step(x_discrete, y_discrete, where='mid', color=color_map[go_id], alpha=0.3, linestyle='--')
        ...
        plt.plot(lowess[:, 0], lowess[:, 1], label=wrapped_label, color=color_map[go_id], linewidth=2.5, zorder=4)
        plt.scatter(x_discrete, y_discrete, color=color_map[go_id], s=50, zorder=5)

    elif valid_mask.sum() > 1:
        plt.step(x_discrete[valid_mask], y_discrete[valid_mask], where='mid', color=color_map[go_id], alpha=0.3, linestyle='--')
        plt.plot(x_discrete[valid_mask], y_discrete[valid_mask], label=wrapped_label, color=color_map[go_id], linewidth=2.5, zorder=4)
        plt.scatter(x_discrete[valid_mask], y_discrete[valid_mask], color=color_map[go_id], s=50, zorder=5)

plt.xticks(ticks=[0, 1, 2], labels=['Past (1 Year Prior)', 'Present (Annotation Date)', 'Future (1 Year After)'], fontsize=16)
plt.yticks(fontsize=16)
plt.title(f"GO {aspect} Trend of Annotated Neighbors Fractions\n(Depth: {depth} - Cutoff: {cutoff})", fontsize=26, fontweight='bold', pad=25)
...
plt.legend(title="GO ID", bbox_to_anchor=(1.05, 1), loc='upper left', title_fontsize=16, fontsize=14)
plt.tight_layout()
...
for year in years:
    ...
    yearly_agg = subset_df.groupby('GO_id')[['annotated_neighbors_present', 'total_neighbors_present']].sum()
    
    for go_id in go_ids:
        if go_id in yearly_agg.index:
            ann_sum = yearly_agg.loc[go_id, 'annotated_neighbors_present']
            tot_sum = yearly_agg.loc[go_id, 'total_neighbors_present']
            frac = ann_sum / tot_sum if tot_sum > 0 else np.nan
        else:
            frac = np.nan
            
        cumulative_records.append({'Year': year, 'GO_id': go_id, 'Fraction': frac})
...
for go_id in go_ids:
    go_data = cum_df[cum_df['GO_id'] == go_id].dropna(subset=['Fraction'])
    ...
    if not go_data.empty:
        ...
        plt.plot(go_data['Year'], go_data['Fraction'], marker='o', markersize=8, 
                 label=wrapped_label, color=color_map[go_id], linewidth=2.5, zorder=4)

plt.xticks(ticks=years, labels=[str(y) for y in years], fontsize=14, rotation=45)
plt.yticks(fontsize=16)
plt.title(f"GO {aspect} Cumulative Trend of Annotated Neighbors Fractions Over Time\n(Depth: {depth} - Cutoff: {cutoff})", fontsize=26, fontweight='bold', pad=25)

...
# Find global bounds to align the plot properly
...
for i, go_id in enumerate(go_ids):
    ...
    # Filter early noise (Wait until 15 edges exist)
...
# High-Detail X-Axis Formatting
ax = plt.gca()
ax.xaxis.set_major_locator(ticker.MultipleLocator(1)) # Label every 1 Year
ax.xaxis.set_minor_locator(ticker.MultipleLocator(0.25)) # Gridlines every 3 months
...
plt.title(f"True Historian Peri-Event Lifespan: Fraction of Annotated Neighbors\n(GO {aspect} - Depth: {depth} - Cutoff: {cutoff})", fontsize=24, fontweight='bold', pad=25)

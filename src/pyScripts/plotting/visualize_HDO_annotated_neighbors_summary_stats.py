...
print(f"Plotting stats for HDO top coefficients annotated neighbors depth {depth} cutoff {cutoff}...")

print(f"Loading data for HDO top coefficients annotated neighbors depth {depth} cutoff {cutoff}...")

stats = pd.read_csv(stats_df, sep = '\t')
ontology = pronto.Ontology(do)

print(f"Plotting HDO top coefficients annotated neighbors depth {depth} cutoff {cutoff} stats table...")
...
plt.title("Top Coefficients HDO Annotated Neighborhood Comparison Statistics", fontweight = 'bold', pad = 30)

print(f"Saving stats table for HDO top coefficients annotated neighbors depth {depth} cutoff {cutoff}...")

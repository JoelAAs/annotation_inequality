...
print(f"Processing top GO {aspect} coefficients depth {depth} cutoff {cutoff} correlation with baseline plotting...")

print(f"Loading top GO {aspect} coefficients depth {depth} cutoff {cutoff} correlation with baseline data...")
...
print(f"Setting up top GO {aspect} coefficients depth {depth} cutoff {cutoff} correlation with baseline data...")
...
stats['GO_term'] = stats['GO_annotation'].apply(get_goid_name)

dist_data = []

for i in range(len(obs_df)):
    annot = obs_df.iloc[i]['annotation']
    name = get_goid_name(annot)
...
print(f"Plotting top GO {aspect} coefficients depth {depth} cutoff {cutoff} correlation with baseline...")

fig, (ax1, ax2) = plt.subplots(2, 1, figsize = (14, 20), sharex = True)
...
for i, row in stats.iterrows():
    ...
    ax1.text(i, h + (max_val * 0.04), p_text, ha = 'center', va = 'bottom', fontsize = 16, fontweight = 'bold')

ax1.set_title(f"A. Mean GO {aspect} Neighbor Bait Counts (Paired Wilcoxon)", loc = 'left', fontweight = 'bold', fontsize = 26, pad = 25)
...
ax2.set_yscale('log')
ax2.set_title(f"B. Distribution of GO {aspect} Neighbor Sums (Log Scale)", loc = 'left', fontweight='bold', fontsize = 26, pad = 25)
ax2.set_ylabel("Neighbors Bait Count Sum (log scale)", fontsize = 22, labelpad = 20)
ax2.set_xlabel(f"GO {aspect} Annotation", fontsize = 22, labelpad = 20)
ax2.set_xticklabels(
...
print(f"Saving Top GO {aspect} coefficients depth {depth} cutoff {cutoff} correlation with baseline plots...")

plt.subplots_adjust(bottom=0.2, hspace=0.3)
plt.savefig(outputplots, dpi = 300, bbox_inches='tight')

print(f"Top GO {aspect} coefficients depth {depth} cutoff {cutoff} correlation with baseline plotting done!")
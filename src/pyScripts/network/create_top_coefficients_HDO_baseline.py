...
print(f"Processing baseline for top HDO depth {depth} cutoff {cutoff} coefficients...")

print(f"Loading HDO depth {depth} cutoff {cutoff} data...")

with open(inputnetwork, 'rb') as f:
    G = pickle.load(f)
coefficients = pd.read_csv(coefficients_df, sep = '\t')

print(f"Computing baseline neighbors bait sum counts for HDO depth {depth} cutoff {cutoff}...")
...
print(f"Saving outputs for HDO depth {depth} cutoff {cutoff} top coefficients baseline...")
...
print(f"Baseline for HDO depth {depth} cutoff {cutoff} top coefficients ready!")